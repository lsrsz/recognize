...
def quad_loss(y_true, y_pred):
    ...
    inside_score_loss = tf.reduce_mean(
        -1 * (beta * labels * tf.compat.v1.log(predicts + cfg.epsilon) +
              (1 - beta) * (1 - labels) * tf.compat.v1.log(1 - predicts + cfg.epsilon)))
    ...
    pos = -1 * vertex_beta * vertex_labels * tf.compat.v1.log(vertex_predicts +
                                                    cfg.epsilon)
    neg = -1 * (1 - vertex_beta) * (1 - vertex_labels) * tf.compat.v1.log(
